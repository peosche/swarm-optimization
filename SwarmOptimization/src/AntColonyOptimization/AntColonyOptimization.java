...
public class AntColonyOptimization {
	...
	public static final double Q0 = 0.5;//paramètre permettant de gérer la part d'exploitation (<= Q0) et d'exploration baisée (> Q0)
	...
	public AntColonyOptimization() {
		this.graph = new SingleGraph("AntColonyOptimization");
	...
	public void initGraph() {
		...
		Generator gen = new RandomGenerator(AntColonyOptimization.DEGREE_GRAPH, false, true);
		gen.addSink(this.graph);
		gen.begin();
		while (this.graph.getNodeCount() < AntColonyOptimization.NB_NODES && gen.nextEvents());
		gen.end();
	}
	...
	public void initAgents() {
		for (int i = 0; i < AntColonyOptimization.NB_AGENTS; i++) {
	...
	public void initPheromones() {
		for(Edge edge : graph.getEachEdge()) {
			edge.addAttribute("pheromone", AntColonyOptimization.INITIAL_PHEROMONE_VALUE);
		}
	}
	...
	public void initDistances() {
		...
		for(Edge e:graph.getEachEdge()) {
			randomDistance = random.nextInt(AntColonyOptimization.MAX_DISTANCE + 1 - AntColonyOptimization.MIN_DISTANCE) + AntColonyOptimization.MIN_DISTANCE;
	...
	public void iteration() {
		...
		for(Agent agent : this.agents) {
			agent.resetTour(AntColonyOptimization.STARTING_NODE);
		}
			...
			for(Agent agent : this.agents) {
				...
				if (!agent.hasFinishedPath() && !this.getPossibleEdges(agent).isEmpty()) {
					...
					if (edge != null) {//si l'agent n'est pas bloqué à son noeud courant
						...
						this.localPheromoneUpdate(edge);
					}
				}
			}
		}
	...
	//basée sur la pseudo-random proportional rule (exploitation vs biased exploration)
	public Edge step(Agent agent) {
		...
		if (!edgesList.isEmpty()) {//s'il existe au moins une arête à emprunter à partir du noeud courant
			double q = Math.random();//variable aléatoire permettant de déterminer si exploitation ou exploration biaisée
			if (q <= AntColonyOptimization.Q0) {//exploitation
				double numerator = 0;
				double bestNumerator = -1.0;
				for (Edge edge : edgesList) {
					numerator = Math.pow((double) edge.getAttribute("pheromone"), AntColonyOptimization.ALPHA)
							* Math.pow((1.0/(double) edge.getAttribute("distance")), AntColonyOptimization.BETA);
					if (numerator > bestNumerator) {
						bestNumerator = numerator;
						chosenEdge = edge;
					}
				}
			} else {//exploration biaisée (idem à AntSystem)
				//calcul de la somme totale du dénominateur
				double denominator = 0;
				for (Edge edge : edgesList) {
					denominator += Math.pow((double) edge.getAttribute("pheromone"), AntColonyOptimization.ALPHA)
							* Math.pow((1.0/(double) edge.getAttribute("distance")), AntColonyOptimization.BETA);
				}
	...
	public void bestTour() {
		...
		for (Agent agent : this.agents) {
			if (agent.hasVisitedNode(AntColonyOptimization.ENDING_NODE)) {//si l'agent a trouvé un chemin pour aller au noeud objectif
	...
	//met à jour les phéromones selon le meilleur parcours de l'itération courante
	//(évaporation pour toutes les arêtes)
	//(dépose uniquement pour les arêtes appartenant au meilleur parcours)
	public void globalPheromoneUpdate() {
		Iterator<Edge> it = this.graph.getEdgeIterator();
		while (it.hasNext()) {//toutes les arêtes
			Edge e = it.next();
			double deltaTau = 0;
			if (this.bestTourEdges.contains(e.getId())) {//si l'arête appartient au meilleur tour
				deltaTau = 1.0 / this.bestTourDistance;
			}
			double oldPheromone = e.getAttribute("pheromone");
			double newPheromone = (1.0 - AntColonyOptimization.RHO) * oldPheromone
					+ AntColonyOptimization.RHO * deltaTau;
	...
	//mise-à-jour "locale" des phéromones à chaque déplacement de chaque agent
	public void localPheromoneUpdate(Edge edge) {
		double deltaTau = AntColonyOptimization.INITIAL_PHEROMONE_VALUE;//une des trois possibilités pour deltaTau
		double oldPheromone = edge.getAttribute("pheromone");
		double newPheromone = (1.0 - AntColonyOptimization.RHO) * oldPheromone
				+ AntColonyOptimization.RHO * deltaTau;
		edge.changeAttribute("pheromone", (double)Math.round(newPheromone * 10000000d) / 10000000d);
		//System.out.println(edge.getId());
		//System.out.println("oldPheromone : "+oldPheromone);
		//System.out.println("newPheromone : "+newPheromone);
	}
	...
	public void fancyDisplayInit() {
        ...
        this.graph.getNode(AntColonyOptimization.STARTING_NODE).addAttribute("ui.class", "start");
        this.graph.getNode(AntColonyOptimization.ENDING_NODE).addAttribute("ui.class", "objective");
        this.graph.display();
	}
